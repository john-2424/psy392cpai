...
Because SR-`wonly`'s greedy success is uneven across seeds and partly transient. Of three seeds: one hits 1.0 and holds it for the whole adaptation window (AUC 1.00), one hits 1.0 mid-window then regresses (AUC 0.14), one never lifts (AUC 0.00). The snapshot only sees the endpoint; the AUC integrates over the window and gives credit for sustained or transient correctness. The methodological lesson is: for architectures where the greedy policy passes through the solution instead of settling in it, AUC is the honest metric. The snapshot alone is misleading.
